...
import (
	...
	"github.com/seed30/TikTok/dal/dao/model"
	model1 "github.com/seed30/TikTok/kitex_gen/douyin/model"
	"errors"
	"regexp"
	"sync"
	"time"

	rpcInteraction "github.com/seed30/TikTok/kitex_gen/douyin/interaction"
	"github.com/seed30/TikTok/pkg/configs"
	"github.com/seed30/TikTok/rpc"
	"github.com/seed30/TikTok/service/interaction"
	"context"
...
var _ = Describe("Comment test", func() {
	...
	Context("Test Comment", func() {
		...
		It("test Query Comment success, but user not exist", func() {
			mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `comment`")).
				WithArgs(video.ID, 0).
				WillReturnRows(sqlmock.NewRows(commentColumns).
					AddRow(1, sTime, 1, 1, "contents1"))
